...
1. Open `/judge-console` and state the product in one line: private aid eligibility with public Stellar settlement accountability.
2. Open `/donor` from the console and show donor escrow funding/receipt.
3. Open `/admin`, enter the private demo `ADMIN_API_SECRET`, issue a wallet-bound credential, and show the downloaded/copied JSON.
...
7. Show the Stellar Explorer transaction, private claim receipt, nullifier, amount, contract, verifier, and Merkle root.
8. Retry the same credential to show nullifier replay rejection.
9. Use `/receipt`, `/impact`, `/protocol`, `/evidence`, and `/audit` only as secondary proof if the reviewer asks for more detail.
10. If a reviewer asks about third-party validation, point them to `docs/THIRD_PARTY_AUDIT_PLAYBOOK.md` and `docs/INDEPENDENT_AUDIT_SCOPE.md`.
